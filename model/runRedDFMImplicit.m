function [uu,yy] = runRedDFMImplicit(p, uu0, T0)
...
while t < Te
    ...
    P = Pc + flipud(cumsum(flipud(dPdx*p.ds)));
...
%     I_vG = cumsum(aG*C0/gamma./max(P,1e4).* dPdx *p.ds);
    % Total gas velocity from adding effect of boundary and injection:
    vG  =  exp(I_vG).*(vG0);
    ...
    for i=1:3
        rho_G = P./p.c_G.^2;
        rho_M = p.rho_L*(1-aG) + rho_G.*aG;

        dFric_dx = F*p.fVec.* vG.*abs(vG)./(p.Dc-p.Dp) .*rho_M;
        dGrav_dx = p.g*sin(p.thetaVec).*rho_M;
        dPdx = dFric_dx+dGrav_dx;

        % Integrated pressure change (integrated from top)
    %     Fric = flipud( cumtrapz(p.s,flipud(dFric_dx)) );
    %     Grav = flipud( cumtrapz(p.s,flipud(dGrav_dx)) );

        P = max(Pc + flipud( cumtrapz(p.s,flipud(dFric_dx+dGrav_dx)) ), 1e5);
%         P = max(Pc + flipud( cumsum(flipud(dFric_dx+dGrav_dx)) )*p.ds, 1e5);

        % Compute gas velocity 
        vG0 = 1/p.A/(1-p.alpha_LS)*(qL+qG) + p.v_inf;
        I_vG = cumtrapz(p.s,aG/(1-p.alpha_LS)/gamma./P.* dPdx);
%         I_vG= cumsum(aG/(1-p.alpha_LS)/gamma./P.* dPdx * p.ds);
        vG  =  exp(I_vG).*(vG0 + cumtrapz(p.s,-C0*aG.*PcDot./(gamma*P)...
            .*exp(-I_vG)) );
        ...
        %     C_K = 1;
            ...
            PcDot2 = barBeta/V*( qL + qG - Qc + Txe );
        else 
            PcDot2 = 0;
        end
        PcDot = PcDot + .5*(PcDot2-PcDot);
    end
    
    %% Compute actual Pressure and Pressure    
    rho_G = P./p.c_G.^2;
    rho_M = p.rho_L*(1-aG) + rho_G.*aG;
...
% %     % Central difference
%     aG_pad = [aG0; aG; 2*aG(end)-aG(end-1)];
%     aGxp = 1/2*(aG_pad(in1+2)-aG_pad(in1));
